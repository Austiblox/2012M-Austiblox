	...
	)
{
...
#if CG
	OUT = (vertexOutputSimple)0;
#endif

#if FXCOMPOSER_VERSION // in fxcompser editor
	IN.Color = float4(gSurfaceColor,1.0);
#endif
	...
	OUT.SurfaceFresnel.xy = IN.SurfaceUV*0.18;
    ...
    OUT.SurfaceFresnel.z = lerp(Kr,KrMin,lerpAlpha);
#else
	OUT.SurfaceFresnel.z = Kr * FresnelMin * 2.0;
#endif
...
void main()
{
	...
	IN.SurfaceUV = gl_MultiTexCoord1.xy;
	IN.Normal = gl_Normal;
	
	// OGRE puts tangent in 6th tex coord because custom GLSL attributes have problems on NV
	IN.Tangent = gl_MultiTexCoord6.xyz;

	vertexOutputSimple OUT = ConcreteVP( IN, WorldITXf, WorldXf, ViewIXf, WvpXf,
	WvXf,
	Lamp0Pos, Lamp1Pos, Kr, FresnelMin, FresnelExp, Lamp0Color, Lamp1Color, AmbiColor );

	gl_Position = OUT.HPosition;
	gl_FrontColor = OUT.DiffuseContrib;
	ModelUV = OUT.ModelUV;;
	Light0Vec = OUT.Light0Vec;
	WorldNormal = OUT.WorldNormal;
	WorldView = OUT.WorldView;
	SurfaceFresnel = OUT.SurfaceFresnel;
}
