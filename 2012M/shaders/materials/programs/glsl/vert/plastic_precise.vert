...
vertexOutputPrecise plastic_vp_precise(appdata IN,
	...
	OUT.TextureUVW = float4(IN.Position.xyz, 0.0);
#if FXCOMPOSER_VERSION // in fxcompser editor
	OUT.Color = float4(gSurfaceColor,1.0);
#else
	OUT.Color = IN.Color;
#endif
	OUT.TextureUVW.w = -vs_compute_gbuffer_depth(IN.Position, WvXf) / GBUFFER_MAX_DEPTH;
...
void main()
{
	...
	vertexOutputPrecise OUT = plastic_vp_precise( IN, WorldITXf, WorldXf, ViewIXf, WvpXf,
	WvXf,
	Lamp0Pos, Lamp1Pos );

	gl_Position = OUT.HPosition;
	gl_FrontColor = OUT.Color;
	ModelUV = OUT.ModelUV;
	TextureUVW = OUT.TextureUVW;
