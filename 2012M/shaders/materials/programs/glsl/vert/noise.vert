...
void main()
{
	...
	IN.Color = gl_Color;
	IN.StudsUV = gl_MultiTexCoord0.xy;
	IN.TexPos3D = gl_MultiTexCoord1;
	IN.SurfaceUV = gl_MultiTexCoord2.xy;
	IN.Normal = gl_Normal;

	// OGRE puts tangent in 6th tex coord because custom GLSL attributes have problems on NV
	IN.Tangent = gl_MultiTexCoord6.xyz;

	VertexOutput OUT = NoiseVS( IN, WorldITXf, WorldXf, ViewIXf, WvpXf,
		WvXf,
		Lamp0Pos, Lamp1Pos, NormMapScale, Transform3DNoiseCoordinates, FadeDistance );
